...
OUTPUT_ROOT = ROOT / "Bending_Torsion_Beam_Test_Case_2nd" / "TestCase_Output" / "Analytical_Finite_Tangent_Work_v2"
BEAM_LENGTH = 10.0
...
def surface_directional_work(surface, plus_displacements, minus_displacements, epsilon):
    work = 0.0
    for node in surface.Nodes:
        force = vector_to_tuple(node.GetSolutionStepValue(KM.REACTION))
        plus = plus_displacements[node.Id]
        minus = minus_displacements[node.Id]
        displacement_derivative = tuple((plus[i] - minus[i]) / (2.0 * epsilon) for i in range(3))
...
def run_case(case_index, kinematics_mode, parameter_value, force_mode, perturbation_mode, output_path):
    ...
    mapper.Map(KM.DISPLACEMENT, KM.ROTATION, KM.DISPLACEMENT, KM.Flags())
    plus_surface_displacements = capture_surface_displacements(surface)
    perturbed_forward_map_time = time.perf_counter() - perturbed_forward_map_start_time
    apply_perturbation(beam, perturbation_mode, -2.0 * FINITE_DIFFERENCE_EPSILON)
    mapper.Map(KM.DISPLACEMENT, KM.ROTATION, KM.DISPLACEMENT, KM.Flags())
    minus_surface_displacements = capture_surface_displacements(surface)
    surface_work = surface_directional_work(
        surface,
        plus_surface_displacements,
        minus_surface_displacements,
